...
main :: IO ()
main = do
    file <- readFile "./../data/input1.txt"
    let input = lines file
    let id_And_GameSets = organise $ map (splitFirst ':') input
    let id_and_amount = calc id_And_GameSets
    print id_and_amount
    let result = filter snd $ map (second check) id_and_amount -- check which Games are valid
    print result
    let sum = foldr (\(a,_) b -> a + b) 0 result -- sum up the id of the valid games
    print sum
